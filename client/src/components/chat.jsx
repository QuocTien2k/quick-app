...
import { faCheckCircle, faFaceSmile, faImage, faSpinner, faTimes } from '@fortawesome/free-solid-svg-icons'
...
const ChatArea = ({ socket, onlineUser }) => {
    ...
    //theo dõi nhận tin
    useEffect(() => {
        if (selectedChat?._id && user && allUsersLoaded && selectedUser) {
    ...
    }, [selectedChat, user, allUsersLoaded, selectedUser]);

    //console.log("Tất cả tin nhắn: ", allMessages);
    //console.log("Danh sách tin nhắn: ", allChats)

    if (!selectedChat) return null; // Không có selectedChat thì ẩn hẳn khung chat

    if (!selectedUser) {
        return <div className="fixed bottom-4 right-4 w-[320px] sm:w-[360px] h-[360px] bg-white rounded-lg shadow-lg flex items-center justify-center z-50">
